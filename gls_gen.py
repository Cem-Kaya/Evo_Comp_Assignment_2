...
class GLS_GEN:
    ...
    For each generation:
        1 Pair up parents 
        2 Produce children via crossover  ( what type ? ? ?  Maybe TODO play with other ttypes beside uniform )
        3 Optimize each child via FM
        4 Combine old population + children -> select top pop_size (Maybe TODO maybe add some randomness here )
    ...
    def run_gls(self):
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            #print(f"Combined population size: {len(combined)}")
            #selection
    ...
    def save_gls_run_to_pickle(self, stats_dict, save_dir="pckl"):
        os.makedirs(save_dir, exist_ok=True)

        timestamp = time.strftime("%Y%m%d_%H%M%S")
        filename = f"gls_run_pop{stats_dict['pop_size']}_iter_count_{stats_dict['max_iterations']}_{timestamp}.pkl"
        filepath = os.path.join(save_dir, filename)

        # Add timestamp to saved content
        stats_dict["timestamp"] = time.strftime("%Y-%m-%d %H:%M:%S")

        with open(filepath, "wb") as f:
            pickle.dump(stats_dict, f)

        print(f"GLS run saved to: {filepath}")
...
if __name__ == "__main__":
    ...
    for pop_size in pop_sizes_to_test:
        ...
        gls.save_gls_run_to_pickle(stats_dict=stats)
